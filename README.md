...
When a genuinely newer *version* of an app exists, the tab shows an **Upgrade**
button for it. A new version may migrate its database on first start, so that
app is backed up before it switches. A new **major version of a database**
(Postgres 14 to 17, say) is listed but never offered as a button: its data
directory belongs to the version that wrote it, and moving between majors is a
migration, not an image swap.
